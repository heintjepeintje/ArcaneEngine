...
namespace Arcane {
	...
	void InitWSA() {
		...
		AR_WINDOWS_ASSERT(WSAStartup(version, &sWSAData) == 0, "Failed to initialize WSA: %d", WSAGetLastError());
	...
	void WindowsSocket::Connect(const std::string &address, uint32_t port) {
		...
		AR_WINDOWS_ASSERT(getaddrinfo(address.c_str(), std::to_string(port).c_str(), &hints, &results) == 0, "Failed to resolve addrinfo: %d", WSAGetLastError());
		...
		AR_WINDOWS_ASSERT(connect(mSocket, results->ai_addr, (int)results->ai_addrlen) != SOCKET_ERROR, "Failed to connect socket: %d", WSAGetLastError());
	...
	void WindowsSocket::Bind(const std::string &address, uint32_t port) {
		...
		AR_WINDOWS_ASSERT(getaddrinfo(address.c_str(), std::to_string(port).c_str(), &hints, &result) == 0, "Failed to resolve addrinfo: %d", WSAGetLastError());

		mSocket = socket(result->ai_family, result->ai_socktype, result->ai_protocol);
		AR_WINDOWS_ASSERT(mSocket != INVALID_SOCKET, "Failed to create socket: %d", WSAGetLastError());

		AR_WINDOWS_ASSERT(bind(mSocket, result->ai_addr, (int)result->ai_addrlen) != SOCKET_ERROR, "Failed to bind socket: %d", WSAGetLastError());
	...
	void WindowsSocket::Listen(uint32_t backlog) {
		AR_WINDOWS_ASSERT(listen(mSocket, SOMAXCONN) != SOCKET_ERROR, "Failed to listen on socket: %d", WSAGetLastError());
	}

	Ref<NativeSocket> WindowsSocket::Accept() {
		SOCKET clientSocket = accept(mSocket, NULL, NULL);
		AR_WINDOWS_ASSERT(clientSocket != INVALID_SOCKET, "Failed to accept connection: %d", WSAGetLastError());
	...
	void WindowsSocket::Send(const void *data, size_t size) {
		...
		AR_WINDOWS_ASSERT(send(mSocket, static_cast<const char*>(data), static_cast<int>(size), 0) != SOCKET_ERROR, "Failed to send data: %d", WSAGetLastError());
	}
	...
	size_t WindowsSocket::Receive(void *buffer, size_t size) {
		...
		AR_WINDOWS_ASSERT(bytesReceived != SOCKET_ERROR, "Failed to receive data: %d", WSAGetLastError());
