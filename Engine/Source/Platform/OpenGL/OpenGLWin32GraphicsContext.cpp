...
namespace Arcane {
	...
	void OnOpenGLDebugMessage(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *message, const void *userParam) {
		switch (severity) {
			case GL_DEBUG_SEVERITY_HIGH: AR_OPENGL_ASSERT(false, "{}", message); break;
			case GL_DEBUG_SEVERITY_MEDIUM: AR_OPENGL_ERROR("{}", message); break;
			// case GL_DEBUG_SEVERITY_LOW: AR_OPENGL_WARNING("{}", message); break;
			case GL_DEBUG_SEVERITY_NOTIFICATION: AR_OPENGL_INFO("{}", message); break;
		}
	}
